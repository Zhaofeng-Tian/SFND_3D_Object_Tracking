...
void computeTTCCamera(std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC, cv::Mat *visImg)
{
    ...
    // Directly use median ID to depress the noise
    double medDistRatio = distRatios[medIndex];
...
{
    //TTC = d1*delat_t / (d0 - d1)
    float delta_t = 1.0 / frameRate;
    float minPrev= 100000.0;
    float minCurr = 100000.0;
    float prev = 0.0;
    float curr = 0.0;
    float vel = 0.0;
    if (lidarPointsPrev.size() == 0 || lidarPointsCurr.size() == 0)
    {
        std::cout << "Lidar Points Set is Empty!" << std::endl;
        TTC = NAN;
        return;
    }
    ...
    std::cout << "Mean Prev: " << prev << std::endl;
    ...
    std::cout << "Mean Curr: " << curr << std::endl;
    vel = (prev - curr) / (delta_t);
    if (vel > 0) TTC = curr * delta_t / (prev - curr);
    else 
    {
        TTC = NAN;
        return;
